...
git fetch
```
Change `fetch` to a bolder operation that requires private access if you feel it. Make sure your file is executable.
From the `user` folder, as the webserver user, at the command prompt enter `.git/hooks/test-ops.sh`. You should see only happy messages.
